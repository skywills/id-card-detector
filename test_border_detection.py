##https://yixiaoyang.github.io/articles/2016-12/opencv-contours
...
cv2.imwrite(os.path.join(OUTPUT_PATH, "1-threshold.png"), binary, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])

binary = cv2.Canny(binary, 0, 60, apertureSize = 3) 
cv2.imwrite(os.path.join(OUTPUT_PATH, "3-canny.png"), binary, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
    ...
    while True:
        ...
        if math.fabs(cv2.contourArea(approx)) > Config.min_area:
            if (len(approx) > 4):
                ...
                cv2.imwrite(os.path.join(OUTPUT_PATH, "transfer%d.png"%idx), warped, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
                break
        else:
            print("failed %d area=%f"%(idx, math.fabs(cv2.contourArea(approx))))
            break

cv2.imwrite(os.path.join(OUTPUT_PATH, "4-contours.png"), binary, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
cv2.imwrite(os.path.join(OUTPUT_PATH, "5-cut.png"), image, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])